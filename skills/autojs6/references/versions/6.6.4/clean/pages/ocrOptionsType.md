...
title: "OcrOptions"
version: "6.6.4"
source_html: "raw/ocrOptionsType.html"
---

# OcrOptions

## OcrOptions {#ocroptionstype_ocroptions}

OcrOptions 是一个代表 OCR 识别选项的接口.

OcrOptions

### [p?] region {#ocroptionstype_p_region}

- { OmniRegion (omniTypes.html#omnitypes_omniregion) }

指定 OCR 识别的区域.

当 `region` 值为 `undefined` 时, 表示不指定区域限制.
当 `region` 值为 `null` 时, 表示空区域, 此时 ocr (ocr.html) 模块的相关方法将返回空结果 (如空数组).

#### number[] {#ocroptionstype_number}
...
#### AndroidRect {#ocroptionstype_androidrect}

也可使用 AndroidRect (androidRectType.html) 表示区域:
