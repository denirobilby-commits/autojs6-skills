---
title: "控件节点行为 (UiObjectActions)"
version: "6.6.4"
source_html: "raw/uiObjectActionsType.html"
---

# 控件节点行为 (UiObjectActions) {#uiobjectactionstype_uiobjectactions}

UiObjectActions 是一个 Java 接口, 代表 控件节点 (UiObject) (uiObjectType.html) 的行为集合.
...
行为 ID 封装方法名 最低 API 等级 ACTION_ACCESSIBILITY_FOCUS accessibilityFocus - ACTION_CLEAR_ACCESSIBILITY_FOCUS clearAccessibilityFocus - ACTION_CLEAR_FOCUS clearFocus - ACTION_CLEAR_SELECTION clearSelection * - ACTION_CLICK click - ACTION_COLLAPSE collapse - ACTION_CONTEXT_CLICK contextClick - ACTION_COPY copy - ACTION_CUT cut - ACTION_DISMISS dismiss - ACTION_DRAG_CANCEL dragCancel * 32 (12.1) [S_V2] ACTION_DRAG_DROP dragDrop * 32 (12.1) [S_V2] ACTION_DRAG_START dragStart * 32 (12.1) [S_V2] ACTION_EXPAND expand - ACTION_FOCUS focus - ACTION_HIDE_TOOLTIP hideTooltip * 28 (9) [P] ACTION_IME_ENTER imeEnter * 30 (11) [R] ACTION_LONG_CLICK longClick - ACTION_MOVE_WINDOW moveWindow * 26 (8) [O] ACTION_NEXT_AT_MOVEMENT_GRANULARITY nextAtMovementGranularity * - ACTION_NEXT_HTML_ELEMENT nextHtmlElement * - ACTION_PAGE_DOWN pageDown * 29 (10) [Q] ACTION_PAGE_LEFT pageLeft * 29 (10) [Q] ACTION_PAGE_RIGHT pageRight * 29 (10) [Q] ACTION_PAGE_UP pageUp * 29 (10) [Q] ACTION_PASTE paste - ACTION_PRESS_AND_HOLD pressAndHold * 30 (11) [R] ACTION_PREVIOUS_AT_MOVEMENT_GRANULARITY previousAtMovementGranularity * - ACTION_PREVIOUS_HTML_ELEMENT previousHtmlElement * - ACTION_SCROLL_BACKWARD scrollBackward - ACTION_SCROLL_DOWN scrollDown - ACTION_SCROLL_FORWARD scrollForward - ACTION_SCROLL_LEFT scrollLeft - ACTION_SCROLL_RIGHT scrollRight - ACTION_SCROLL_TO_POSITION scrollTo - ACTION_SCROLL_UP scrollUp - ACTION_SELECT select - ACTION_SET_PROGRESS setProgress - ACTION_SET_SELECTION setSelection - ACTION_SET_TEXT setText - ACTION_SHOW_ON_SCREEN show - ACTION_SHOW_TEXT_SUGGESTIONS showTextSuggestions * 33 (13) [TIRAMISU] ACTION_SHOW_TOOLTIP showTooltip * 28 (9) [P]
...
## [m!] performAction {#uiobjectactionstype_m_performaction}

用于执行指定的控件行为.
是一个无默认实现的抽象方法.

### performAction(action, ...arguments) {#uiobjectactionstype_performaction_action_arguments}

**`A11Y`**

- **action** { number (dataTypes.html#datatypes_number) } - 行为的唯一标志符 (Action ID)
- **arguments** { ... (documentation.html#documentation_可变参数)ActionArgument[] (documentation.html#documentation_可变参数) } - 行为参数, 用于给行为传递参数
- **returns** { boolean (dataTypes.html#datatypes_boolean) } - 是否行为已执行且执行过程中无异常

实现此方法的类有:

- UiSelector (uiSelectorType.html)
- UiObject (uiObjectType.html)
- UiObjectCollection (uiObjectCollectionType.html)
...
由此可见, UiSelector (uiSelectorType.html) 与 UiObjectCollection (uiObjectCollectionType.html) 最终都调用了 UiObject (uiObjectType.html) 的 `performAction` 方法, 而 `UiObject` 的 `performAction` 则调用了 Android 系统的 AccessibilityNodeInfoCompat#performAction (https://developer.android.com/reference/androidx/core/view/accessibility/AccessibilityNodeInfoCompat#performAction(int,android.os.Bundle)) 方法

`UiObjectCollection` 相当于对控件集合中的每一个控件执行 `UiObject#performAction` 方法.
`UiSelector` 相当于先执行 untilFind (uiSelectorType.html#uiselectortype_m_untilfind) 找到当前窗口中所有控件, 将其作为集合执行 `UiObjectCollection#performAction` 方法.
...
`UiObject` 拥有另一个实例方法 performAction(action, bundle) (uiObjectType.html#uiobjecttype_performactionaction_bundle), 因此上述示例也可改写为:
...
如需判断控件是否有某个或某些 `Action`, 可使用 UiObject#hasAction (uiObjectType.html#uiobjecttype_m_hasaction):
...
如需使用 `Action` 选择器筛选控件, 可使用 UiSelector#action (uiSelectorType.html#uiselectortype_m_action):
...
## [m=] click {#uiobjectactionstype_m_click}

### click() {#uiobjectactionstype_click}
...
鉴于上述优劣项, 控件的 `click` 方法通常与 [ global.click (global.html#global_m_click) (automator.click (automator.html#automator_m_click)) / UiObject#clickByBounds (uiObjectType.html#uiobjecttype_m_clickbybounds) ] 等方法配合使用.

## [m=] longClick {#uiobjectactionstype_m_longclick}

### longClick() {#uiobjectactionstype_longclick}
...
## [m=] accessibilityFocus {#uiobjectactionstype_m_accessibilityfocus}

### accessibilityFocus() {#uiobjectactionstype_accessibilityfocus}
...
## [m=] clearAccessibilityFocus {#uiobjectactionstype_m_clearaccessibilityfocus}

### clearAccessibilityFocus() {#uiobjectactionstype_clearaccessibilityfocus}
...
## [m=] focus {#uiobjectactionstype_m_focus}

### focus() {#uiobjectactionstype_focus}
...
## [m=] clearFocus {#uiobjectactionstype_m_clearfocus}

### clearFocus() {#uiobjectactionstype_clearfocus}
...
## [m=] dragStart {#uiobjectactionstype_m_dragstart}

### dragStart() {#uiobjectactionstype_dragstart}
...
## [m=] dragDrop {#uiobjectactionstype_m_dragdrop}

### dragDrop() {#uiobjectactionstype_dragdrop}
...
## [m=] dragCancel {#uiobjectactionstype_m_dragcancel}

### dragCancel() {#uiobjectactionstype_dragcancel}
...
## [m=] imeEnter {#uiobjectactionstype_m_imeenter}

### imeEnter() {#uiobjectactionstype_imeenter}
...
## [m=] moveWindow {#uiobjectactionstype_m_movewindow}

### moveWindow(x, y) {#uiobjectactionstype_movewindow_x_y}
...
## [m=] nextAtMovementGranularity {#uiobjectactionstype_m_nextatmovementgranularity}

### nextAtMovementGranularity(granularity, isExtendSelection) {#uiobjectactionstype_nextatmovementgranularity_granularity_isextendselection}
...
## [m=] nextHtmlElement {#uiobjectactionstype_m_nexthtmlelement}

### nextHtmlElement(element) {#uiobjectactionstype_nexthtmlelement_element}
...
## [m=] pageLeft {#uiobjectactionstype_m_pageleft}

### pageLeft() {#uiobjectactionstype_pageleft}
...
## [m=] pageUp {#uiobjectactionstype_m_pageup}

### pageUp() {#uiobjectactionstype_pageup}
...
## [m=] pageRight {#uiobjectactionstype_m_pageright}

### pageRight() {#uiobjectactionstype_pageright}
...
## [m=] pageDown {#uiobjectactionstype_m_pagedown}

### pageDown() {#uiobjectactionstype_pagedown}
...
## [m=] pressAndHold {#uiobjectactionstype_m_pressandhold}

### pressAndHold() {#uiobjectactionstype_pressandhold}
...
## [m=] previousAtMovementGranularity {#uiobjectactionstype_m_previousatmovementgranularity}

### previousAtMovementGranularity(granularity, isExtendSelection) {#uiobjectactionstype_previousatmovementgranularity_granularity_isextendselection}
...
## [m=] previousHtmlElement {#uiobjectactionstype_m_previoushtmlelement}

### previousHtmlElement(element) {#uiobjectactionstype_previoushtmlelement_element}
...
## [m=] showTextSuggestions {#uiobjectactionstype_m_showtextsuggestions}

### showTextSuggestions() {#uiobjectactionstype_showtextsuggestions}
...
## [m=] showTooltip {#uiobjectactionstype_m_showtooltip}

### showTooltip() {#uiobjectactionstype_showtooltip}
...
## [m=] hideTooltip {#uiobjectactionstype_m_hidetooltip}

### hideTooltip() {#uiobjectactionstype_hidetooltip}
...
## [m=] show {#uiobjectactionstype_m_show}

### show() {#uiobjectactionstype_show}
...
## [m=] dismiss {#uiobjectactionstype_m_dismiss}

### dismiss() {#uiobjectactionstype_dismiss}
...
## [m=] copy {#uiobjectactionstype_m_copy}

### copy() {#uiobjectactionstype_copy}
...
## [m=] cut {#uiobjectactionstype_m_cut}

### cut() {#uiobjectactionstype_cut}
...
## [m=] paste {#uiobjectactionstype_m_paste}

### paste() {#uiobjectactionstype_paste}
...
详情参阅 getClip (global.html#global_m_getclip).

## [m=] select {#uiobjectactionstype_m_select}

### select() {#uiobjectactionstype_select}
...
## [m=] expand {#uiobjectactionstype_m_expand}

### expand() {#uiobjectactionstype_expand}
...
## [m=] collapse {#uiobjectactionstype_m_collapse}

### collapse() {#uiobjectactionstype_collapse}
...
## [m=] scrollLeft {#uiobjectactionstype_m_scrollleft}

### scrollLeft() {#uiobjectactionstype_scrollleft}
...
## [m=] scrollUp {#uiobjectactionstype_m_scrollup}

### scrollUp() {#uiobjectactionstype_scrollup}
...
## [m=] scrollRight {#uiobjectactionstype_m_scrollright}

### scrollRight() {#uiobjectactionstype_scrollright}
...
## [m=] scrollDown {#uiobjectactionstype_m_scrolldown}

### scrollDown() {#uiobjectactionstype_scrolldown}
...
## [m=] scrollForward {#uiobjectactionstype_m_scrollforward}

### scrollForward() {#uiobjectactionstype_scrollforward}
...
## [m=] scrollBackward {#uiobjectactionstype_m_scrollbackward}

### scrollBackward() {#uiobjectactionstype_scrollbackward}
...
## [m=] scrollTo {#uiobjectactionstype_m_scrollto}

### scrollTo(row, column) {#uiobjectactionstype_scrollto_row_column}
...
## [m=] contextClick {#uiobjectactionstype_m_contextclick}

### contextClick() {#uiobjectactionstype_contextclick}
...
## [m=] setText {#uiobjectactionstype_m_settext}

### setText(text) {#uiobjectactionstype_settext_text}
...
## [m=] setSelection {#uiobjectactionstype_m_setselection}

### setSelection(start, end) {#uiobjectactionstype_setselection_start_end}
...
## [m=] clearSelection {#uiobjectactionstype_m_clearselection}

### clearSelection() {#uiobjectactionstype_clearselection}
...
## [m=] setProgress {#uiobjectactionstype_m_setprogress}

### setProgress(progress) {#uiobjectactionstype_setprogress_progress}
...
## [I] ActionArgument {#uiobjectactionstype_i_actionargument}

控件的行为参数接口.
主要用于自定义控件行为的 performAction 抽象方法内.

### [C] IntActionArgument {#uiobjectactionstype_c_intactionargument}

ActionArgument 的具体类.
用于传递 Int 类型的行为参数.

#### [c] (name, value) {#uiobjectactionstype_c_name_value}

- **name** { string (dataTypes.html#datatypes_string) } - 行为名称
- **value** { number (dataTypes.html#datatypes_number) } - 行为参数值
...
### [C] BooleanActionArgument {#uiobjectactionstype_c_booleanactionargument}

ActionArgument 的具体类.
用于传递 Boolean 类型的行为参数.

#### [c] (name, value) {#uiobjectactionstype_c_name_value_1}

- **name** { string (dataTypes.html#datatypes_string) } - 行为名称
- **value** { boolean (dataTypes.html#datatypes_boolean) } - 行为参数值
- **returns** { ActionArgument }
...
### [C] CharSequenceActionArgument {#uiobjectactionstype_c_charsequenceactionargument}

ActionArgument 的具体类.
用于传递 CharSequence 类型的行为参数.

#### [c] (name, value) {#uiobjectactionstype_c_name_value_2}

- **name** { string (dataTypes.html#datatypes_string) } - 行为名称
- **value** { string (dataTypes.html#datatypes_string) } - 行为参数值
- **returns** { ActionArgument }
...
### [C] StringActionArgument {#uiobjectactionstype_c_stringactionargument}

ActionArgument 的具体类.
用于传递 String 类型的行为参数.

#### [c] (name, value) {#uiobjectactionstype_c_name_value_3}
...
### [C] FloatActionArgument {#uiobjectactionstype_c_floatactionargument}

ActionArgument 的具体类.
用于传递 Float 类型的行为参数.

#### [c] (name, value) {#uiobjectactionstype_c_name_value_4}
...
Global Actions AUTO SEL accessibilityFocus √ clearAccessibilityFocus √ clearFocus √ clearSelection √ click √ collapse √ contextClick √ copy √ cut √ dismiss √ dragCancel √ dragDrop √ dragStart √ expand √ focus √ hideTooltip √ imeEnter √ longClick √ moveWindow √ nextAtMovementGranularity √ nextHtmlElement √ pageDown √ pageLeft √ pageRight √ pageUp √ paste √ pressAndHold √ previousAtMovementGranularity √ previousHtmlElement √ scrollBackward √ scrollDown √ scrollForward √ scrollLeft √ scrollRight √ scrollTo √ scrollUp √ select √ setProgress √ setSelection √ setText √ show √ showTextSuggestions √ showTooltip √
...
有关全局方法 `paste` 的执行过程及原理分析可参阅 UiSelector#paste (uiSelectorType.html#uiselectortype_m_paste) 小节.
