...
title: "CryptoKey"
version: "6.6.4"
source_html: "raw/cryptoKeyType.html"
---

# CryptoKey {#cryptokeytype_cryptokey}

CryptoKey 是 `org.autojs.autojs.core.crypto.Crypto.Key` 的子类, 其实例可代表一个密钥 (确切地说, 是自定义密钥, 而非 Java 密钥), 主要用于 密文 (crypto.html) 模块.

常见相关方法或属性:

- CryptoKeyPair#publicKey (cryptoKeyPairType.html#cryptokeypairtype_p_publickey)
- CryptoKeyPair#privateKey (cryptoKeyPairType.html#cryptokeypairtype_p_privatekey)
- crypto.Key (crypto.html#crypto_c_key)::new

注: 本章节仅列出 CryptoKey 独有的而不包含继承的属性及方法.

CryptoKey

## [p] data {#cryptokeytype_p_data}

**`READONLY`**

- { ByteArray (dataTypes.html#datatypes_bytearray) } - Java 字节数组
...
## [p] keyPair {#cryptokeytype_p_keypair}
...
需特别留意, 与 Auto.js Pro 不同, `keyPair` 默认值为 `null`, 而非 `undefined`. 这是因为 AutoJs6 的 crypto (crypto.html) 模块底层实现不是 JavaScript 语言.

## [m] toKeySpec {#cryptokeytype_m_tokeyspec}

### toKeySpec(transformation) {#cryptokeytype_tokeyspec_transformation}
