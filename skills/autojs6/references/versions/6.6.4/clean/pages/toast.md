...
title: "消息浮动框 (Toast)"
version: "6.6.4"
source_html: "raw/toast.html"
---

# 消息浮动框 (Toast) {#toast_toast}
...
部分机型的 toast 消息正常显示依赖通知权限, 当未授予通知权限或通知被 `阻止 (block)` 时, toast 可能无法正常显示, 参阅 notice.isEnabled (notice.html#notice_m_isenabled) 小节.
...
### toast(text, isLong) {#toast_toast_text_islong}

**`Global`** **`Overload 2/4`** **`Async`**

- **text** { string (dataTypes.html#datatypes_string) } - 消息内容
- **isLong = false** { `'long'` | `'l'` | `'short'` | `'s'` | boolean (dataTypes.html#datatypes_boolean) } - 是否以较长时间显示
- **returns** { void (dataTypes.html#datatypes_void) }
...
### toast(text, isLong, isForcible) {#toast_toast_text_islong_isforcible}

**`Global`** **`Overload 3/4`** **`Async`**

- **text** { string (dataTypes.html#datatypes_string) } - 消息内容
- **isLong = false** { `'long'` | `'l'` | `'short'` | `'s'` | boolean (dataTypes.html#datatypes_boolean) } - 是否以较长时间显示
- **isForcible = false** { `'forcible'` | `'f'` | boolean (dataTypes.html#datatypes_boolean) } - 是否强制覆盖显示
- **returns** { void (dataTypes.html#datatypes_void) }
...
### toast(text, isForcible) {#toast_toast_text_isforcible}
...
## [m] dismissAll {#toast_m_dismissall}

### dismissAll() {#toast_dismissall}
