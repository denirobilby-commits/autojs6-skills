...
version: "6.6.4"
source_html: "raw/dataTypes.html"
---

# 数据类型 (Data Types) {#datatypes_data_types}

此章节待补充或完善...

Marked by SuperMonster003 on Feb 22, 2023.
...
- `NaN` - 特殊数值
- 参阅: NaN (glossaries.html#glossaries_nan)
...
注: 如需表示一个很大的数 (超过 `2^53 - 1`), 需要用 BigInt (glossaries.html#glossaries_bigint) 表示.
文档中通常不会出现 `bigint` 类型的数据, 包括 `number | bigint` 这样的 联合类型 数据.

## String {#datatypes_string}
...
后缀 "[]" 代表数组类型.
如 `number[]` 代表一个数组, 其中的元素全部为 number 类型, 且元素数量不限 (包括 0, 即空数组).

注: `number[]` 与 `[number]` 不同, 后者表示 元组类型.

注: 使用 `Array<T>` 这样的 泛型 表示法也可代表数组类型, 但文档通常只采用后缀表示法.
...
上述 方法签名 (documentation.html#documentation_方法签名) 中, bar 为函数参数, 该函数是一个无参函数且返回值为 number 类型.
...
- **bar** { (a: string, b: any) => string }

上述 方法签名 (documentation.html#documentation_方法签名) 中, bar 为函数参数, 该函数包含两个参函数且返回值为 string 类型.
...
上述 方法签名 (documentation.html#documentation_方法签名) 中, bar 为正则表达式参数, 是 JavaScript 标准 RegExp 类型:
...
上述 方法签名 (documentation.html#documentation_方法签名) 中, bar 为函数参数,
...
### Void 与 Undefined {#datatypes_void_undefined}
...
在 JavaScript 中, 没有 return 语句的函数将默认返回 undefined.
...
## Never {#datatypes_never}

## Object {#datatypes_object}

### 字面量对象类型 {#datatypes_2}

{ { a: number }}

## Generic {#datatypes_generic}
...
## RegExPattern {#datatypes_regexpattern}
...
## keyof {#datatypes_keyof}

## typeof {#datatypes_typeof}

## extends {#datatypes_extends}

## index {#datatypes_index}

## condition {#datatypes_condition}

## readonly {#datatypes_readonly}

# 操纵泛型 {#datatypes_5}

例如 Array.

## Uppercase {#datatypes_uppercase}
...
## Capitalize {#datatypes_capitalize}
...
## IgnoreCase {#datatypes_ignorecase}
...
## Pattern {#datatypes_pattern}

**Pattern<T extends RegExPattern>: string**

通常用于输入检查.
接受 正则表达式字面量 (glossaries.html#glossaries_正则表达式) 并生成通过测试的 string 类型数据.

Pattern 的泛型通配符 T 在文档中也称作 字符串模式 (glossaries.html#glossaries_字符串模式).
...
为便于理解或重复引用, 有些 Pattern 类型会被重新定义为自定义类型, 如 NumberString (dataTypes.html#datatypes_numberstring).
...
上述示例的 bar 参数接受除 number 外的任意类型.

# 自定义类型 {#datatypes_6}

## JavaArray {#datatypes_javaarray}
...
## JavaArrayList {#datatypes_javaarraylist}

Java ArrayList (Java 数组列表).

与 Java Array 不同的是, ArrayList 创建的数组可调整大小:
...
## NumberString {#datatypes_numberstring}

数字字符串.

字符串模式 (glossaries.html#glossaries_字符串模式): `/[+-]?(\d+(\.\d+)?(e\d+)?)/`.
...
## ComparisonOperatorString {#datatypes_comparisonoperatorstring}

比较操作符字符串.

字符串模式 (glossaries.html#glossaries_字符串模式): `/<=?|>=?|=/`.
...
## ScreenMetricNumberX {#datatypes_screenmetricnumberx}
...
**bottom** { ScreenMetricNumberX (dataTypes.html#datatypes_screenmetricnumberx) }
...
## ScreenMetricNumberY {#datatypes_screenmetricnumbery}
...
**top** { ScreenMetricNumberY (dataTypes.html#datatypes_screenmetricnumbery) }
...
## ScriptExecuteActivity {#datatypes_scriptexecuteactivity}
...
## DetectCompass {#datatypes_detectcompass}

用于传递给 控件罗盘 (uiObjectType.html#uiobjecttype_m_compass) 的参数类型, 又称 `罗盘参数`.

罗盘参数是 字符串 (dataTypes.html#datatypes_string) 类型, 支持单独或组合使用.
...
控件罗盘 (UiObject.compass) (uiObjectType.html#uiobjecttype_m_compass) 是 控件探测 (UiObject.detect) (uiObjectType.html#uiobjecttype_m_detect) 的衍生方法, 因此类型命名采用了 `DetectCompass`.

## DetectResult {#datatypes_detectresult}

控件探测 (UiObject.detect) (uiObjectType.html#uiobjecttype_m_detect) 的结果参数类型, 又称 `探测结果`, 此过程也称为 `结果筛选`.

- `# / w / widget` - 控件 (uiObjectType.html)
- `$ / txt / content` - 文本内容 (uiObjectType.html#uiobjecttype_m_content)
- `. / pt / point` - 点 (uiObjectType.html#uiobjecttype_m_point)
...
不同于 PickupResult (拾取结果), `探测结果` 的种类相对较少.

## DetectCallback {#datatypes_detectcallback}

探测回调.

探测回调用于处理 控件探测 (UiObject.detect) (uiObjectType.html#uiobjecttype_m_detect) 的结果.
...
## PickupSelector {#datatypes_pickupselector}

拾取选择器 (uiSelectorType.html#uiselectortype_m_pickup) 的 `选择器参数`.

`选择器参数` 的类型分为 单一型选择器 和 混合型选择器.

### 单一型选择器 {#datatypes_7}

单一型选择器包含 [ 经典选择器 / 内容选择器 / 对象选择器 ].
...
上述示例的选择器参数使用了混合型选择器, 它包含两个单一型选择器, 分别为 内容选择器 和 经典选择器.
...
## PickupResult {#datatypes_pickupresult}

拾取选择器 (UiSelector#pickup) (uiSelectorType.html#uiselectortype_m_pickup) 的结果参数类型, 又称 `拾取结果`, 此过程也称为 `结果筛选`.

- `# / w / widget` - 控件 (UiObject) (uiObjectType.html)
- `{} / #{} / {#} / w{} / {w} / wc / collection / list` -> 控件集合 (UiObjectCollection) (uiObjectCollectionType.html)
- `[] / #[] / [#] / w[] / [w] / ws / widgets` -> 控件 (UiObject) (uiObjectType.html) 数组
- `$ / txt / content` - 文本内容 (UiObject#content) (uiObjectType.html#uiobjecttype_m_content)
- `$[] / [$] / txt[] / [txt] / content[] / [content] / contents` -> 文本内容 (UiObject#content) (uiObjectType.html#uiobjecttype_m_content) 数组
- `. / pt / point` - 点 (UiObject#point) (uiObjectType.html#uiobjecttype_m_point)
- `.[] / [.] / point[] / [point] / pt[] / [pt] / points / pts` -> 点 (UiObject#point) (uiObjectType.html#uiobjecttype_m_point) 数组
- `@ / selector / sel` -> 选择器 (UiSelector) (uiSelectorType.html)
- `? / exists` -> 存在判断 (UiSelector#exists) (uiSelectorType.html#uiselectortype_m_exists)
- `UiObjectInvokable` - 控件可调用类型
...
与 DetectResult (探测结果) 相比, `拾取结果` 的种类更加丰富.

## UiObjectInvokable {#datatypes_uiobjectinvokable}

控件可调用类型, 用于使用参数形式实现方法调用, 又称 `参化调用`.

支持所有 UiObject (uiObjectType.html) 的实例方法, 如果方法需要传递参数, 需要将参数连同方法名称放入数组后再传递.
...
## RootMode {#datatypes_rootmode}

Root 模式, 枚举类型, 已全局化.

枚举实例名 描述 JavaScript 代表参数 AUTO_DETECT 自动检测 Root 权限 'auto' / -1 FORCE_ROOT 强制 Root 模式 'root' / 1 / true FORCE_NON_ROOT 强制非 Root 模式 'non-root' / 0 / false
...
## ColorInt {#datatypes_colorint}

颜色整数 (Color Integer).

多数情况下, 使用颜色整数代表一个颜色.
在安卓源码中, 颜色整数用 `ColorInt` 表示, 其值的范围由 `Java` 的 `Integer` 类型决定, 即 `[-2^31..2^31-1]`.
例如数字 `0xBF110523` 对应十进制的 `3205563683`, 超出了上述 `ColorInt` 的范围, 因此相关方法 (如 colors.toInt (color.html#color_m_toint)) 会将此数值通过 `2^32` 偏移量移动至合适的范围内, 最终得到结果 `-1089403613`.
...
如需将 `-52480` 这样的值还原为具有可读性的颜色代码, 可使用 colors.toHex (color.html#color_m_tohex) 等方法.

## ColorName {#datatypes_colorname}

颜色名称.

颜色列表 (Color Table) (colorTable.html) 章节中, 各个颜色列表中 "变量名" 的字符串形式可直接作为颜色名称使用:
...
当使用 `颜色名称 (ColorName)` 作为参数时, 同一个名称可能同时出现在不同的 颜色列表 (colorTable.html) 中, 如 `CYAN` 在所有列表中均有出现, 且 Material 颜色列表 (colorTable.html#colortable_material_颜色列表) 中的 `CYAN` 与其它列表中的 `CYAN` 颜色不同.
...
详情参阅 颜色列表 (Color Table) (colorTable.html) 章节的 颜色名称冲突 (colorTable.html#colortable_颜色名称冲突) 小节.
...
## ColorComponent {#datatypes_colorcomponent}
...
样例 等效语句 备注 colors.rgb(64, 32, 224) - - colors.rgba(64, 32, 224, 255) - - colors.hsv(30, 20, 60) colors.hsv(30, 0.2, 0.6) S (saturation) 和 V (value) 分量范围为 [0..100] colors.hsva(30, 20, 60, 255) colors.hsva(30, 0.2, 0.6, 255) A (alpha) 分量范围为 [0..255]

**2. 浮点数**

样例 等效语句 备注 colors.rgb(0.5, 0.25, 0.125) colors.rgb(128, 64, 32) - colors.rgba(0.5, 0.25, 0.1, 0.2) colors.rgba(128, 64, 26, 51) - colors.hsv(10, 0.3, 0.2) colors.hsv(10, 30, 20) - colors.hsva(10, 0.3, 0.2, 0.5) colors.hsva(10, 30, 20, 128) 不同分量的范围不同

**3. 百分数**

样例 等效语句 备注 colors.rgb('50%', '25%', '12.5%') colors.rgb(128, 64, 32) - colors.rgba('50%', '25%', '10%', '20%') colors.rgba(128, 64, 26, 51) - colors.hsv(10, '30%', '20%') colors.hsv(10, 30, 20) - colors.hsva(10, '30%', '20%', '50%') colors.hsva(10, 30, 20, 128) 不同分量的范围不同

### 表示范围 {#datatypes_15}

不同分量的范围不同, 当使用浮点数或百分数等表示法时, 需留意其表示范围:

分量 范围 R (red) [0..255] G (green) [0..255] B (blue) [0..255] A (alpha) [0..255] H (hue) [0..360] S (saturation) [0..100] V (value) [0..100] L (lightness) [0..100]
...
## ColorComponents {#datatypes_colorcomponents}
...
colors 全局对象的很多 "to" 开头的方法都可返回颜色分量数组, 如 toRgb (color.html#color_m_torgb), toHsv (color.html#color_m_tohsv), toHsl (color.html#color_m_tohsl), toRgba (color.html#color_m_torgba), toArgb (color.html#color_m_toargb) 等.

需额外注意 toRgba (color.html#color_m_torgba) 和 toArgb (color.html#color_m_toargb) 结果中的 `A (alpha)` 分量, 默认范围为 `[0..255]`, 而其他方法则恒为 `[0..1]`:
...
## ColorDetectionAlgorithm {#datatypes_colordetectionalgorithm}
...
### RGB 差值检测 {#datatypes_rgb_1}

参数名称: `diff`

计算两个 RGB 颜色各分量的差值:

### RGB 距离检测 {#datatypes_rgb_2}

参数名称: `rgb`

计算 RGB 色彩空间中两点间距离:

### 加权 RGB 距离检测 {#datatypes_rgb_3}
...
### HS 距离检测 {#datatypes_hs}

参数名称: `hs`

HSV 色彩空间中 `H (hue)` 及 `S (saturation)` 的相关距离检测:

## Range {#datatypes_range}

表示一个数字的数值范围.

表示法 范围 (a..b) { x | a < x < b } [a..b] { x | a <= x <= b } (a..b] { x | a < x <= b } [a..b) { x | a <= x < b } (a..+∞) { x | x > a } [a..+∞) { x | x >= a } (-∞..b) { x | x < b } (-∞..b] { x | x <= b } (-∞..+∞) { x } (任意值)

如 `Range[10..30]` 表示数字 `x` 位于 `10 <= x <= 30` 范围内, 而 `Range[0..1)` 表示数字 `x` 位于 `0 <= x < 1` 范围内.

## IntRange {#datatypes_intrange}

表示一个整数的取值范围. 其表示法可参阅 Range 小节.

如 `IntRange[10..30]` 表示整数 `x` 位于 `10 <= x <= 30` 范围内, 而 `IntRange[0..100)` 表示整数 `x` 位于 `0 <= x < 100` 范围内.

## StandardCharset {#datatypes_standardcharset}

StandardCharset 类型支持 Java 字符集 (Charset 类) 形式及字符串形式:

Charset String Wikipedia ISO_8859_1 "ISO_8859_1" / "iso-8859-1" 英 (https://en.wikipedia.org/wiki/ISO/IEC_8859-1) / 中 (https://zh.wikipedia.org/zh-hans/ISO/IEC_8859-1) US_ASCII "US_ASCII" / "us-ascii" 英 (https://en.wikipedia.org/wiki/ASCII) / 中 (https://zh.wikipedia.org/wiki/ASCII) UTF_8 "UTF_8" / "utf-8" 英 (https://en.wikipedia.org/wiki/UTF-8) / 中 (https://zh.wikipedia.org/wiki/UTF-8) UTF_16 "UTF_16" / "utf-16" 英 (https://en.wikipedia.org/wiki/UTF-16) / 中 (https://zh.wikipedia.org/wiki/UTF-16) UTF_16BE "UTF_16BE" / "utf-16be" 英 (https://en.wikipedia.org/wiki/UTF-16#Byte-order_encoding_schemes) UTF_16LE "UTF_16LE" / "utf-16le" 英 (https://en.wikipedia.org/wiki/UTF-16#Byte-order_encoding_schemes)
...
## ExtendModulesNames {#datatypes_extendmodulesnames}

AutoJs6 内置扩展插件 (plugins.html#plugins_内置扩展插件) 的插件名称.
...
## ActivityShortForm {#datatypes_activityshortform}
...
## BroadcastShortForm {#datatypes_broadcastshortform}
...
## OcrModeName {#datatypes_ocrmodename}

AutoJs6 的 OCR 模式名称.

当使用不同的模式名称时, `ocr` 全局方法及其相关方法 (如 ocr.detect (ocr.html#ocr_m_detect)) 将使用不同的引擎, 进而可能获得不同的识别速度和结果.

- `mlkit` - 代表 MLKit 引擎
- `paddle` - 代表 Paddle Lite 引擎

## OcrResult {#datatypes_ocrresult}

OcrResult 是一个代表 OCR 识别结果的接口.

OcrResult

### [p] label {#datatypes_p_label}
...
### [p] confidence {#datatypes_p_confidence}
...
### [p] bounds {#datatypes_p_bounds}

- { AndroidRect (androidRectType.html) }

OCR 识别结果的位置矩形, 用 AndroidRect (androidRectType.html) 表示.
...
### [m] toString {#datatypes_m_tostring}

#### toString() {#datatypes_tostring}
...
## ThemeColor {#datatypes_themecolor}
...
- autojs.themeColor (autojs.html#autojs_p_themecolor)
- Color (colorType.html)(**themeColor**)

当 ThemeColor 作为 OmniColor (omniTypes.html#omnitypes_omnicolor) 使用时, 将使用其 "主色" 作为色值:
...
## JsByteArray {#datatypes_jsbytearray}

JavaScript 用于表示 "字节数组" 的类型, 即 number (dataTypes.html#datatypes_number)[] (dataTypes.html#datatypes_array).
...
## CryptoDigestAlgorithm {#datatypes_cryptodigestalgorithm}

密文 (crypto.html) 模块使用的消息摘要算法.

值 (字符串) MD5 SHA-1 SHA-224 SHA-256 SHA-384 SHA-512
...
参阅: Oracle Docs (https://docs.oracle.com/en/java/javase/11/docs/specs/security/standard_names.html#https://docs.oracle.com/en/java/javase/11/docs/specs/security/standard_names_keypairgenerator_algorithms) / 常用消息摘要算法简介 (http://www.semlinker.com/message-digest-intro)

## CryptoKeyPairGeneratorAlgorithm {#datatypes_cryptokeypairgeneratoralgorithm}

密文 (crypto.html) 模块使用的密钥对生成器算法.

值 (字符串) 别名 DH DiffieHellman DSA - RSA - EC - XDH -

例如, AutoJs6 的 crypto.generateKeyPair (crypto.html#crypto_m_generatekeypair) 方法可以生成用于非对称加密算法的公私密钥对:
...
参阅: Oracle Docs (https://docs.oracle.com/en/java/javase/11/docs/specs/security/standard_names.html#https://docs.oracle.com/en/java/javase/11/docs/specs/security/standard_names_keypairgenerator_algorithms)

## CryptoDigestOptions {#datatypes_cryptodigestoptions}

消息摘要生成选项, 主要用于 密文 (crypto.html) 模块.

属性 有效值 简述 input 'file' / 'base64' / 'hex' / **'string'** 指定输入类型. output 'bytes' / 'base64' / **'hex'** / 'string' 指定输出类型. encoding 'US-ASCII' / 'ISO-8859-1' / **'UTF-8'**
'UTF-16BE' / 'UTF-16LE' / 'UTF-16' 指定输入或输出编码,
仅对 'string' 类型有效.

注: 上表中粗体值为属性默认值.

## CryptoCipherTransformation {#datatypes_cryptociphertransformation}

密码转换名称, 主要用于 密文 (crypto.html) 模块.

Cipher, 可翻译为 "密码" 或 "密码器".

AutoJs6 的 crypto (crypto.html) 模块中, encrypt (crypto.html#crypto_m_encrypt) 和 decrypt (crypto.html#crypto_m_decrypt) 的内部实现均借助了 `javax.crypto.Cipher` 实例.
...
- crypto.encrypt (crypto.html#crypto_m_encrypt)(data, key, **transformation**, options?)
- crypto.decrypt (crypto.html#crypto_m_decrypt)(data, key, **transformation**, options?)

下表列出了 AutoJs6 支持的转换名称构成要素, 可组合出多种不同的转换名称:

算法名称 工作模式 填充方式 AES CBC
CFB
CTR
CTS
ECB
OFB ISO10126Padding
NoPadding
PKCS5Padding AES GCM NoPadding AES_128 CBC
ECB NoPadding
PKCS5Padding AES_128 GCM NoPadding AES_256 CBC
ECB NoPadding
PKCS5Padding AES_256 GCM NoPadding ARC4 ECB NoPadding ARC4 NONE NoPadding BLOWFISH CBC
CFB
CTR
CTS
ECB
OFB ISO10126Padding
NoPadding
PKCS5Padding ChaCha20 NONE
Poly1305 NoPadding DES CBC
CFB
CTR
CTS
ECB
...
## Storage {#datatypes_storage}

参阅 Storage - 存储类 (storageType.html) 类型章节.

## AndroidBundle {#datatypes_androidbundle}

参阅 AndroidBundle (androidBundleType.html) 类型章节.

## AndroidRect {#datatypes_androidrect}

参阅 AndroidRect (androidRectType.html) 类型章节.

## CryptoCipherOptions {#datatypes_cryptocipheroptions}

参阅 CryptoCipherOptions (cryptoCipherOptionsType.html) 类型章节.

## ConsoleBuildOptions {#datatypes_consolebuildoptions}

参阅 ConsoleBuildOptions (consoleBuildOptionsType.html) 类型章节.

## HttpRequestBuilderOptions {#datatypes_httprequestbuilderoptions}

参阅 HttpRequestBuilderOptions (httpRequestBuilderOptionsType.html) 类型章节.

## HttpRequestHeaders {#datatypes_httprequestheaders}

参阅 HttpRequestHeaders (httpRequestHeadersType.html) 类型章节.

## HttpResponseBody {#datatypes_httpresponsebody}

参阅 HttpResponseBody (httpResponseBodyType.html) 类型章节.

## HttpResponseHeaders {#datatypes_httpresponseheaders}

参阅 HttpResponseHeaders (httpResponseHeadersType.html) 类型章节.

## HttpResponse {#datatypes_httpresponse}

参阅 HttpResponse (httpResponseType.html) 类型章节.

## InjectableWebClient {#datatypes_injectablewebclient}

参阅 InjectableWebClient (injectableWebClientType.html) 类型章节.

## InjectableWebView {#datatypes_injectablewebview}

参阅 InjectableWebView (injectableWebViewType.html) 类型章节.

## NoticeOptions {#datatypes_noticeoptions}

参阅 NoticeOptions (noticeOptionsType.html) 类型章节.

## NoticeChannelOptions {#datatypes_noticechanneloptions}

参阅 NoticeChannelOptions (noticeChannelOptionsType.html) 类型章节.

## NoticePresetConfiguration {#datatypes_noticepresetconfiguration}

参阅 NoticePresetConfiguration (noticePresetConfigurationType.html) 类型章节.

## NoticeBuilder {#datatypes_noticebuilder}

参阅 NoticeBuilder (noticeBuilderType.html) 类型章节.

## Okhttp3HttpUrl {#datatypes_okhttp3httpurl}

参阅 Okhttp3HttpUrl (okhttp3HttpUrlType.html) 类型章节.

## OcrOptions {#datatypes_ocroptions}

参阅 OcrOptions (ocrOptionsType.html) 类型章节.

## Okhttp3Request {#datatypes_okhttp3request}

参阅 Okhttp3Request (okhttp3RequestType.html) 类型章节.

## OpenCVPoint {#datatypes_opencvpoint}

参阅 OpenCVPoint (opencvPointType.html) 类型章节.

## OpenCVRect {#datatypes_opencvrect}

参阅 OpenCVRect (opencvRectType.html) 类型章节.

## OpenCVSize {#datatypes_opencvsize}

参阅 OpenCVSize (opencvSizeType.html) 类型章节.

## OpenCCConversion {#datatypes_openccconversion}

参阅 OpenCCConversion (openCCConversionType.html) 类型章节.
