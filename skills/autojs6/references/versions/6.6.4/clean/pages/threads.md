...
title: "线程 (Threads)"
version: "6.6.4"
source_html: "raw/threads.html"
---

# 线程 (Threads) {#threads_threads}
...
## threads.start(action) {#threads_threads_start_action}

- `action` { Function (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function) } 要在新线程执行的函数
- 返回 Thread
...
## threads.shutDownAll() {#threads_threads_shutdownall}

停止所有通过`threads.start()`启动的子线程.

## threads.currentThread() {#threads_threads_currentthread}

- 返回 Thread

返回当前线程.

## threads.disposable() {#threads_threads_disposable}

- 返回 Disposable

新建一个Disposable对象, 用于等待另一个线程的某个一次性结果. 更多信息参见线程通信以及Disposable.

## threads.atomic([initialValue]) {#threads_threads_atomic_initialvalue}

- `initialValue` { number (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type) } 初始整数值, 默认为0
- 返回AtomicLong (https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/atomic/AtomicLong.html)

新建一个整数原子变量. 更多信息参见线程安全以及AtomicLong (https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/atomic/AtomicLong.html).

## threads.lock() {#threads_threads_lock}

- 返回ReentrantLock (https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/locks/ReentrantLock.html)

新建一个可重入锁. 更多信息参见线程安全以及ReentrantLock (https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/locks/ReentrantLock.html).
...
## Thread.interrupt() {#threads_thread_interrupt}

中断线程运行.

## Thread.join([timeout]) {#threads_thread_join_timeout}

- `timeout` { number (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type) } 等待时间, 单位毫秒
...
## Thread.setTimeout(callback, delay[, ...args]) {#threads_thread_settimeout_callback_delay_args}

参见timers.setTimeout() (timers.html#timers_timers_settimeout_callback_delay_args).
...
## Thread.setInterval(callback, delay[, ...args]) {#threads_thread_setinterval_callback_delay_args}

参见timers.setInterval() (timers.html#timers_timers_setinterval_callback_delay_args).

区别在于, 该定时器会在该线程执行. 如果当前线程仍未开始执行或已经执行结束, 则抛出`IllegalStateException`.

## Thread.setImmediate(callback[, ...args]) {#threads_thread_setimmediate_callback_args}

参见timers.setImmediate() (timers.html#timers_timers_setimmediate_callback_delay_args).

区别在于, 该定时器会在该线程执行. 如果当前线程仍未开始执行或已经执行结束, 则抛出`IllegalStateException`.

## Thread.clearInterval(id) {#threads_thread_clearinterval_id}

参见timers.clearInterval() (timers.html#timers_timers_clearinterval_id).

区别在于, 该定时器会在该线程执行. 如果当前线程仍未开始执行或已经执行结束, 则抛出`IllegalStateException`.

## Thread.clearTimeout(id) {#threads_thread_cleartimeout_id}

参见timers.clearTimeout() (timers.html#timers_timers_cleartimeout_id).

区别在于, 该定时器会在该线程执行. 如果当前线程仍未开始执行或已经执行结束, 则抛出`IllegalStateException`.

## Thread.clearImmediate(id) {#threads_thread_clearimmediate_id}

参见timers.clearImmediate() (timers.html#timers_timers_clearimmediate_id).
...
## sync(func) {#threads_sync_func}

- `func` { Function (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function) } 函数
