...
title: "事件监听 (Events)"
version: "6.6.4"
source_html: "raw/events.html"
---

# 事件监听 (Events) {#events_events}
...
## events.emitter() {#events_events_emitter}

返回一个新的EventEmitter. 这个EventEmitter没有内置任何事件.

## events.observeKey() {#events_events_observekey}
...
## events.onKeyDown(keyName, listener) {#events_events_onkeydown_keyname_listener}

- `keyName` { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } 要监听的按键名称
- `listener` { Function (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function) } 按键监听器. 参数为一个KeyEvent.
...
## events.onKeyUp(keyName, listener) {#events_events_onkeyup_keyname_listener}
...
## events.onceKeyDown(keyName, listener) {#events_events_oncekeydown_keyname_listener}
...
## events.onceKeyUp(keyName, listener) {#events_events_oncekeyup_keyname_listener}
...
## events.removeAllKeyDownListeners(keyName) {#events_events_removeallkeydownlisteners_keyname}

- `keyName` { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } 按键名称

删除该按键的KeyDown(按下)事件的所有监听.

## events.removeAllKeyUpListeners(keyName) {#events_events_removeallkeyuplisteners_keyname}

- `keyName` { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } 按键名称

删除该按键的KeyUp(弹起)事件的所有监听.

## events.setKeyInterceptionEnabled([key, ]enabled) {#events_events_setkeyinterceptionenabled_key_enabled}

- `enabled` { boolean (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Boolean_type) }
- `key` { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } 要屏蔽的按键
...
## events.observeTouch() {#events_events_observetouch}
...
## events.setTouchEventTimeout(timeout) {#events_events_settoucheventtimeout_timeout}

- `timeout` { number (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type) } 两个触摸事件的最小间隔. 单位毫秒. 默认为10毫秒. 如果number小于0, 视为0处理.
...
## events.getTouchEventTimeout() {#events_events_gettoucheventtimeout}

返回触摸事件的最小时间间隔.

## events.onTouch(listener) {#events_events_ontouch_listener}

- `listener` { Function (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function) } 参数为Point (images.html#images_images_point)的函数
...
## events.removeAllTouchListeners() {#events_events_removealltouchlisteners}

删除所有事件监听函数.

## 事件: 'key' {#events_key}

- `keyCode` { number (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type) } 键值
- `event` { KeyEvent } 事件
...
## events.observeNotification() {#events_events_observenotification}
...
## events.observeToast() {#events_events_observetoast}

开启Toast监听.

Toast监听依赖于无障碍服务, 因此此函数会确保无障碍服务运行.

## 事件: 'toast' {#events_toast}

- `toast` { Object (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object) }
...
## 事件: 'notification' {#events_notification}
...
# Notification {#events_notification_1}

通知对象, 可以获取通知详情, 包括通知标题、内容、发出通知的包名、时间等, 也可以对通知进行操作, 比如点击、删除.

## Notification.number {#events_notification_number}

- { number (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type) }

通知数量. 例如QQ连续收到两条消息时number为2.

## Notification.when {#events_notification_when}
...
## Notification.getPackageName() {#events_notification_getpackagename}

- 返回 { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) }

获取发出通知的应用包名.

## Notification.getTitle() {#events_notification_gettitle}

- 返回 { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) }

获取通知的标题.

## Notification.getText() {#events_notification_gettext}

- 返回 { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) }

获取通知的内容.

## Notification.click() {#events_notification_click}

点击该通知. 例如对于一条QQ消息, 点击会进入具体的聊天界面.

## Notification.delete() {#events_notification_delete}

删除该通知. 该通知将从通知栏中消失.

# KeyEvent {#events_keyevent}

## KeyEvent.getAction() {#events_keyevent_getaction}
...
## KeyEvent.getKeyCode() {#events_keyevent_getkeycode}
...
## KeyEvent.getEventTime() {#events_keyevent_geteventtime}

- 返回 { number (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type) }

返回事件发生的时间戳.

## KeyEvent.getDownTime() {#events_keyevent_getdowntime}

返回最近一次按下事件的时间戳. 如果本身是按下事件, 则与`getEventTime()`相同.

## KeyEvent.keyCodeToString(keyCode) {#events_keyevent_keycodetostring_keycode}
...
## EventEmitter.addListener(eventName, listener) {#events_eventemitter_addlistener_eventname_listener}
...
## EventEmitter.emit(eventName[, ...args]) {#events_eventemitter_emit_eventname_args}
...
## EventEmitter.eventNames() {#events_eventemitter_eventnames}
...
## EventEmitter.getMaxListeners() {#events_eventemitter_getmaxlisteners}

返回 EventEmitter 当前的最大监听器限制值, 该值可以通过 emitter.setMaxListeners(n) 设置或默认为 EventEmitter.defaultMaxListeners.

## EventEmitter.listenerCount(eventName) {#events_eventemitter_listenercount_eventname}

- `eventName` { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } 正在被监听的事件名

返回正在监听名为 eventName 的事件的监听器的数量.

## EventEmitter.listeners(eventName) {#events_eventemitter_listeners_eventname}
...
## EventEmitter.on(eventName, listener) {#events_eventemitter_on_eventname_listener}
...
## EventEmitter.once(eventName, listener) {#events_eventemitter_once_eventname_listener}
...
## EventEmitter.prependListener(eventName, listener) {#events_eventemitter_prependlistener_eventname_listener}
...
## EventEmitter.prependOnceListener(eventName, listener) {#events_eventemitter_prependoncelistener_eventname_listener}
...
## EventEmitter.removeAllListeners([eventName]) {#events_eventemitter_removealllisteners_eventname}
...
## EventEmitter.removeListener(eventName, listener) {#events_eventemitter_removelistener_eventname_listener}
...
## EventEmitter.setMaxListeners(n) {#events_eventemitter_setmaxlisteners_n}
...
# events.broadcast: 脚本间广播 {#events_events_broadcast}
