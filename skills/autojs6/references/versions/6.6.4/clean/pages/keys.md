...
## powerDialog() {#keys_powerdialog}

- 返回 { boolean (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Boolean_type) }

弹出电源键菜单. 返回是否执行成功. 此函数依赖于无障碍服务.

## notifications() {#keys_notifications}

- 返回 { boolean (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Boolean_type) }

拉出通知栏. 返回是否执行成功. 此函数依赖于无障碍服务.

## quickSettings() {#keys_quicksettings}

- 返回 { boolean (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Boolean_type) }

显示快速设置(下拉通知栏到底). 返回是否执行成功. 此函数依赖于无障碍服务.

## recents() {#keys_recents}

- 返回 { boolean (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Boolean_type) }

显示最近任务. 返回是否执行成功. 此函数依赖于无障碍服务.

## splitScreen() {#keys_splitscreen}
...
## VolumeUp() {#keys_volumeup}

按下音量上键. 此函数依赖于root权限.

## VolumeDown() {#keys_volumedown}
...
## Text(text) {#keys_text_text}

- text { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } 要输入的文字, 只能为英文或英文符号 输入文字text. 例如`Text("aaa");`

## KeyCode(code) {#keys_keycode_code}

- code { number (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type) } | 要按下的按键的数字代码或名称. 参见下表. 模拟物理按键. 例如`KeyCode(29)`和`KeyCode("KEYCODE_A")`是按下A键.

# 附录: KeyCode对照表 {#keys_keycode}
