...
title: "关于文档 (About)"
version: "6.6.4"
source_html: "raw/documentation.html"
---

# 关于文档 (About) {#documentation_about}
...
## 文档阅读示例 {#documentation}

### 基础 {#documentation_1}

#### device.height {#documentation_device_height}

device 表示全局对象 (这里同时也是一个模块).
".height" 表示访问 device 对象的 height 成员变量.
如 console.log(device.height) 表示在控制台打印当前设备的高度数值.

#### colors.rgb(red, green, blue) {#documentation_colors_rgb_red_green_blue}
...
### 临时作用域对象 {#documentation_2}

通常每个章节都以某个对象作为主题.

例如上述 `colors.rgb(red, green, blue)` 位于 Color - 颜色 (color.html) 这个章节.
...
### 参数类型 {#documentation_3}

#### colors.rgb(red, green, blue) {#documentation_colors_rgb_red_green_blue_1}

- **red** { number (dataTypes.html#datatypes_number) }
- **green** { number (dataTypes.html#datatypes_number) }
- **blue** { number (dataTypes.html#datatypes_number) }

参数后的 "{}" 内包含其类型.
上述示例表示需要传入三个 number (dataTypes.html#datatypes_number) 类型的参数.
如 colors.rgb(255, 128, 64) 合法, 而 colors.rgb("abc", 128, 64) 将可能导致非预期结果或出现异常.

注: 点击类型对应的超链接 (如有) 可跳转至类型详情页面.

### 返回值类型 {#documentation_4}

#### colors.rgb(red, green, blue) {#documentation_colors_rgb_red_green_blue_2}
...
returns 后的 "{}" 内包含返回值类型.
上述示例表示 colors.rgb 方法调用后将返回 number (dataTypes.html#datatypes_number) 类型数据.

### 属性类型 {#documentation_5}

#### colors.RED {#documentation_colors_red}

- { number (dataTypes.html#datatypes_number) }

属性类型包裹在一对花括号中.
上述示例表示 colors 的 RED 属性是 number (dataTypes.html#datatypes_number) 类型数据.

对象字面量形式的类型则用一对双花括号表示:

- **properties** { { name: string (dataTypes.html#datatypes_string); age: number (dataTypes.html#datatypes_number) }}

多行形式:

- **properties** {{
- name: string (dataTypes.html#datatypes_string);
- age: number (dataTypes.html#datatypes_number);
- laugh(decibel?: number (dataTypes.html#datatypes_number));
...
- [ `1200` ] { number (dataTypes.html#datatypes_number) }

上述示例表示一个默认值为 1200 的可存取属性.

以一对双方括号表示常量:

- [[ 0.5 ]] { number (dataTypes.html#datatypes_number) }

上述示例表示一个值为 0.5 的常量属性.

### 方法签名 {#documentation_6}
...
### 方法描述 {#documentation_7}

#### colors.rgb(red, green, blue) {#documentation_colors_rgb_red_green_blue_3}
...
### 可变参数 {#documentation_8}

#### files.join(parent, ...child) {#documentation_files_join_parent_child}
...
### 可选参数 {#documentation_9}

#### device.vibrate(text, delay?) {#documentation_device_vibrate_text_delay}
...
- **[ delay ]** { number (dataTypes.html#datatypes_number) }

如果可选参数包含默认值, 则会以 "=" 标注:

- **[ delay = 0 ]** { number (dataTypes.html#datatypes_number) }

详见下述 参数默认值

### 参数默认值 {#documentation_10}

#### device.vibrate(text, delay?) {#documentation_device_vibrate_text_delay_1}

- **text** { string (dataTypes.html#datatypes_string) } - 需转换为摩斯码的文本
- **[ delay = 0 ]** { number (dataTypes.html#datatypes_number) } - 振动延迟
...
### 方法重载 {#documentation_11}

**`Overload 1/17`**

#### pickup(selector, compass, resultType) {#documentation_pickup_selector_compass_resulttype}

**`Overload 2/17`**

#### pickup(selector, compass) {#documentation_pickup_selector_compass}

**`Overload 3/17`**

#### pickup(selector, resultType) {#documentation_pickup_selector_resulttype}

... ...

**`Overload 16/17`**

#### pickup(root, selector) {#documentation_pickup_root_selector}

**`Overload 17/17`**

#### pickup() {#documentation_pickup}
...
### 方法全局化 {#documentation_12}

**`Global`**

#### images.requestScreenCapture(landscape) {#documentation_images_requestscreencapture_landscape}
...
### 方法标签 {#documentation_13}

用于简便表示方法的特性:

- `Global`: 方法全局化 (可省略模块对象直接调用).
- `Overload 2/3`: 方法重载 [ 第 2 个, 共 3 个 ].
- `Non-UI`: 方法不能在 UI 模式下调用.
- `6.2.0`: 方法对 AutoJs6 的版本要求 [ 不低于 6.2.0 ].
- `[6.2.0]`: 与原同名方法相比, 方法的功能, 结果, 签名或使用方式发生变更的起始版本.
- `API>=29`: 方法对 API 级别 (apiLevel.html) 的要求 [ 不低于 29 ], 当不满足时不抛出异常.
- `API>=29!`: 方法对 API 级别 (apiLevel.html) 的要求 [ 不低于 29 ], 当不满足时将抛出异常.
- `A11Y`: 方法依赖无障碍服务.
- `A11Y?`: 方法可能会依赖无障碍服务.
- `Async`: 异步执行的方法.
...
### 对象标签 {#documentation_14}

用于简便表示对象的属性:

- [m]: 普通对象方法或类静态成员方法.
- 例如在 `images` 作为 临时作用域对象 时:
...
- 这个章节有两个 临时作用域对象, apple 和 banana, 对应 `[1]` 和 `[5]`.
...
### 模板参数 {#documentation_16}

#### foo.bar(a, b) {#documentation_foo_bar_a_b}

- **a** { T (dataTypes.html#datatypes_generic) }
- **b** { number (dataTypes.html#datatypes_number) }
- **returns** { T (dataTypes.html#datatypes_generic) }
- **template** { T (dataTypes.html#datatypes_generic) }
...
参阅: 泛型 (dataTypes.html#datatypes_generic)
