...
title: "异常 (Exceptions)"
version: "6.6.4"
source_html: "raw/exceptions.html"
---

# 异常 (Exceptions) {#exceptions_exceptions}
...
用 `try...catch` 语句可以捕获并处理异常, 详见下述 异常处理 小节.

在 `catch {}` (即 `catch 块`) 中, `e` 对象可用于获取异常相关信息,
而 Rhino 引擎重新包装了 `e` 对象, 通过 `e.javaException` 和 `e.rhinoException` 可获取更多异常信息,
详见下述 catch 块 小节.

# 错误类型 {#exceptions}

## JavaScript {#exceptions_javascript}
...
### RangeError {#exceptions_rangeerror}
...
### ReferenceError {#exceptions_referenceerror}
...
### SyntaxError {#exceptions_syntaxerror}
...
### TypeError {#exceptions_typeerror}
...
### InternalError {#exceptions_internalerror}
...
### EvalError {#exceptions_evalerror}
...
## Rhino {#exceptions_rhino}

Rhino 异常可以视为特殊的 Java 异常, 由 Rhino 运行时 (Runtime) (runtime.html) 包装为对象, 指向 catch 块中异常对象的 rhinoException 属性.
...
## try...catch 语句 {#exceptions_try_catch}
...
## [@] Error {#exceptions_error_2}

## [p#] name {#exceptions_p_name}
...
## [p#] message {#exceptions_p_message}
...
## [p#] stack {#exceptions_p_stack}

- { string (dataTypes.html#datatypes_string) | any (dataTypes.html#datatypes_any) }
...
## [p#] lineNumber {#exceptions_p_linenumber}
...
## [p#] fileName {#exceptions_p_filename}
...
## [p#] javaException {#exceptions_p_javaexception}
...
## [p#] rhinoException {#exceptions_p_rhinoexception}
...
## [m#] toString {#exceptions_m_tostring}

### toString() {#exceptions_tostring}
...
## [m#] toSource {#exceptions_m_tosource}

### toSource() {#exceptions_tosource}
...
## [p] stackTraceLimit {#exceptions_p_stacktracelimit}

- [ `Infinity` ] { number (dataTypes.html#datatypes_number) }
...
## [m] prepareStackTrace {#exceptions_m_preparestacktrace}
...
### prepareStackTrace(errorObj, stackTraces) {#exceptions_preparestacktrace_errorobj_stacktraces}

- **errorObj** { Error } - 异常对象的引用
- **stackTraces** { NodeJS.CallSite (https://microsoft.github.io/PowerBI-JavaScript/interfaces/_node_modules__types_node_globals_d_.nodejs.callsite.html)[] (dataTypes.html#datatypes_array) } - 栈追踪数组 (栈帧组)
...
## [m] captureStackTrace {#exceptions_m_capturestacktrace}

附加 stack 属性 (栈追踪信息) 到任意对象上.

参阅: V8 Dev (https://v8.dev/docs/stack-trace-api)

### captureStackTrace(errorObj, constructorOpt) {#exceptions_capturestacktrace_errorobj_constructoropt}

- **errorObj** { object (dataTypes.html#datatypes_object) } - 用于附加 stack 属性的任意对象
- **constructorOpt** { Function (dataTypes.html#datatypes_function) } - 用于在栈追踪信息中触发隐藏的函数
