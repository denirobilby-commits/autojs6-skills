...
第三行`bg="#ff0000"`指定了垂直布局的背景色(bg)为"#ff0000", 这是一个RGB颜色, 表示红色(有关RGB的相关知识参见RGB颜色对照表 (http://tool.oschina.net/commons?type=3)). 第四行的`textSize="20sp"`则指定了按钮控件的字体大小(textSize)为"20sp", sp是一个字体单位, 暂时不用深入理会. 上述代码的效果如图：
...
## attr(name, value) {#ui_attr_name_value}

- `name` { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } 属性名称
- `value` { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } 属性的值
...
宽度属性也可以指定一个具体数值. 例如`w="20"`, `w="20px"`等. 不加单位的情况下默认单位为dp, 其他单位包括px(像素), mm(毫米), in(英寸). 有关尺寸单位的更多内容, 参见尺寸的单位: Dimension.
...
宽度属性也可以指定一个具体数值. 例如`h="20"`, `h="20px"`等. 不加单位的情况下默认单位为dp, 其他单位包括px(像素), mm(毫米), in(英寸). 有关尺寸单位的更多内容, 参见尺寸的单位: Dimension.
...
## layout_gravity {#ui_layout_gravity}
...
## transformPivotX {#ui_transformpivotx}
...
## transformPivotY {#ui_transformpivoty}
...
# 输入框控件: input {#ui_input}

输入框控件也是一个特殊的文本控件, 因此所有文本控件的函数的属性和函数都适用于按钮控件. 输入框控件有自己的属性和函数, 要查看所有这些内容, 阅读EditText (http://www.zhdoc.net/android/reference/android/widget/EditText.html).
...
## textColorHint {#ui_textcolorhint}

指定输入提示的字体颜色.

## textSizeHint {#ui_textsizehint}

指定输入提示的字体大小.

## inputType {#ui_inputtype}
...
这里只介绍他的主要方法和属性, 如果要查看他的所有方法和属性, 阅读ImageView (http://www.zhdoc.net/android/reference/android/widget/ImageView.html).
...
## radiusTopLeft {#ui_radiustopleft}

图片控件的左上角圆角的半径. 有关该属性的单位, 参见尺寸的单位: Dimension.

## radiusTopRight {#ui_radiustopright}

图片控件的右上角圆角的半径. 有关该属性的单位, 参见尺寸的单位: Dimension.

## radiusBottomLeft {#ui_radiusbottomleft}

图片控件的左下角圆角的半径. 有关该属性的单位, 参见尺寸的单位: Dimension.

## radiusBottomRight {#ui_radiusbottomright}
...
## layout_weight {#ui_layout_weight}
...
# 相对布局: relative {#ui_relative}

# 勾选框控件: checkbox {#ui_checkbox}

# 选择框控件: radio {#ui_radio}

# 选择框布局: radiogroup {#ui_radiogroup}

# 开关控件: Switch {#ui_switch}
...
# 进度条控件: progressbar {#ui_progressbar}

# 拖动条控件: seekbar {#ui_seekbar}

# 下来菜单控件: spinner {#ui_spinner}

# 时间选择控件: timepicker {#ui_timepicker}

# 日期选择控件: datepicker {#ui_datepicker}

# 浮动按钮控件: fab {#ui_fab}

# 标题栏控件: toolbar {#ui_toolbar}

# 卡片: card {#ui_card}

卡片控件是一个拥有圆角、阴影的控件.

## cardBackgroundColor {#ui_cardbackgroundcolor}

卡片的背景颜色.

## cardCornerRadius {#ui_cardcornerradius}

卡片的圆角半径.

## cardElevation {#ui_cardelevation}

设置卡片在z轴上的高度, 来控制阴影的大小.

## contentPadding {#ui_contentpadding}
...
## ui.layout(xml) {#ui_ui_layout_xml}

- `xml` { XML } | { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } 布局XML或者XML字符串

将布局XML渲染为视图（View）对象, 并设置为当前视图.

## ui.layoutFile(xmlFile) {#ui_ui_layoutfile_xmlfile}

- `xml` { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } 布局XML文件的路径

此函数和`ui.layout`相似, 只不过允许传入一个xml文件路径来渲染布局.

## ui.inflate(xml[, parent = null, attachToParent = false]) {#ui_ui_inflate_xml_parent_null_attachtoparent_false}

- `xml` { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } | { XML } 布局XML或者XML字符串
- `parent` { View } 父视图
- `attachToParent` { boolean (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Boolean_type) } 是否渲染的View加到父视图中, 默认为false
- 返回 { View }
...
# ui.registerWidget(name, widget) {#ui_ui_registerwidget_name_widget}

- `name` { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } 组件名称
- `widget` { Function (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function) } 组件

注册一个自定义组件. 参考示例->界面控件->自定义控件.

# ui.isUiThread() {#ui_ui_isuithread}
...
## ui.findView(id) {#ui_ui_findview_id}

- `id` { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } View的ID
- 返回 { View }
...
## ui.finish() {#ui_ui_finish}

结束当前活动并销毁界面.

## ui.setContentView(view) {#ui_ui_setcontentview_view}

- `view` { View }

将视图对象设置为当前视图.

## ui.post(callback[, delay = 0]) {#ui_ui_post_callback_delay_0}

- `callback` { Function (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function) } 回调函数
- `delay` { number (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type) } 延迟, 单位毫秒
...
## ui.run(callback) {#ui_ui_run_callback}
...
## ui.statusBarColor(color) {#ui_ui_statusbarcolor_color}

- color { string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) } | { number (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type) } 颜色
...
## ui.useAndroidResources() {#ui_ui_useandroidresources}
...
# 尺寸的单位: Dimension {#ui_dimension}

# Drawables {#ui_drawables}
