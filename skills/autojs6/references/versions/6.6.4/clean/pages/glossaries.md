...
title: "术语 (Glossaries)"
version: "6.6.4"
source_html: "raw/glossaries.html"
---

# 术语 (Glossaries) {#glossaries_glossaries}
...
参阅: runtime (runtime.html) (全局对象)

## 上下文 {#glossaries_8}

Context.

参阅: context (context.html) (全局对象)
...
## 内置对象扩展 {#glossaries_14}

对 内置对象 的扩展主要有两种类型, 属性扩展及原型扩展.
...
而针对 ECMAScript 规范中的新功能的扩展, 则被称为 填泥 (Polyfill).
...
- 无意中修改或覆盖了 JavaScript 标准的内置方法
- 自定义扩展方法被定义者或合作开发者修改后需修改大量依赖代码甚至出错
- 导入库时可能存在与本地同名扩展方法冲突
- 导入多个库时可能存在库之间的同名扩展方法冲突
- 未来标准更新后可能与已有扩展方法冲突
...
AutoJs6 默认提供了一些内置对象扩展, 如 [ Arrayx (arrayx.html), Numberx (numberx.html), Mathx (mathx.html) ] 等.
...
## Polyfill {#glossaries_polyfill}

又称 [ 代码填泥 / 填泥 / 腻子 / 泥子 ], 是一个完整的代码块, 用于为不支持原生 ECMAScript 新功能的环境提供功能支持.
详见 代码填泥 (polyfill.html) 章节.

## Shim {#glossaries_shim}
...
Shim 与 Polyfill 的不同, 可参阅 代码填泥 (polyfill.html) 章节.
...
将应用资源分离之后, 可使用项目的 R 类中生成的 资源 ID 对其进行访问.
...
将 `资源 ID` 的十六进制值与 `0x` 前缀组合, 可作为控件的 idHex (uiObjectType.html#uiobjecttype_m_idhex) 信息:
...
在 AutoJs6 中, 控件由 UiObject (uiObjectType.html) 表示.
...
## 子项信息集控件 {#glossaries_18}

子项信息集控件指拥有 无障碍节点子项信息集 (AccessibilityNodeInfo.CollectionItemInfo) (https://developer.android.com/reference/android/view/accessibility/AccessibilityNodeInfo.CollectionItemInfo) 实例的一组控件, 它们共同属于同一个 信息集控件.
...
## 控件矩形外展 {#glossaries_19}

控件矩形 (androidRectType.html) 边界为非整数时, 外展将对各个边界做如下取整操作:
...
### 颜色匹配阈值 {#glossaries_23}

取值范围: IntRange[0..255]

参数类型与此类阈值相关的常用方法:

- colors.isSimilar (color.html#color_m_issimilar)
- images.findColor
- images.findColorInRegion
- images.findMultiColors
- images.detectsColor

### 图像匹配阈值 {#glossaries_24}
...
### 图像阈值化阈值 {#glossaries_25}

取值范围: IntRange[0..255]

参数类型与此类阈值相关的常用方法:

- images.threshold(a, b, ***threshold***, c)
...
术语 常用译名 性质 可测量或计算 Luminance 亮度 物理量 √ Brightness 视亮度 感知量 × Lightness 明度 感知量 ×

### Luminance {#glossaries_luminance}
...
Lightness 和 Brightness 用于表示人眼对光亮的实际感受.
...
### Brightness {#glossaries_brightness}
...
### Lightness {#glossaries_lightness}
...
占位符 简述 示例 示例结果 %s 字符串占位符 "状态: %s", "开启"
"状态: %s", 100 "状态: 开启"
"状态: 100" %d 数字占位符 "数量: %d", 5
"数量: %d", "hello" "数量: 5"
"数量: NaN" %j JSON 对象占位符 "o: %j", { a: 1, b: 2 } 'o: { "a":1,"b":2 }' %% 转义 % 符号
%% 将转换为 % "1%% is 0.01"
"1%%%% is 0.0001" "1% is 0.01"
"1%% is 0.0001"
...
## HTTP 标头 {#glossaries_http}

参阅 HTTP 标头 (httpHeaderGlossary.html) 术语章节.

## MIME 类型 {#glossaries_mime}

参阅 MIME 类型 (mimeTypeGlossary.html) 术语章节.
