...
title: "NoticeBuilder"
version: "6.6.4"
source_html: "raw/noticeBuilderType.html"
---

# NoticeBuilder {#noticebuildertype_noticebuilder}
...
- notice.getBuilder (notice.html#notice_m_getbuilder)

注: 本章节仅列出部分属性或方法.

androidx.core.app.NotificationCompat.Builder

## [m] setAutoCancel {#noticebuildertype_m_setautocancel}

### setAutoCancel(autoCancel) {#noticebuildertype_setautocancel_autocancel}

- **autoCancel** { boolean (dataTypes.html#datatypes_boolean) }
- **returns** { NoticeBuilder (noticeBuilderType.html) }

配置用户点击通知后是否自动移除通知.

## [m] setChannelId {#noticebuildertype_m_setchannelid}

### setChannelId(channelId) {#noticebuildertype_setchannelid_channelid}

- **channelId** { string (dataTypes.html#datatypes_string) }
- **returns** { NoticeBuilder (noticeBuilderType.html) }

设定通知渠道 ID.

## [m] setColor {#noticebuildertype_m_setcolor}

### setColor(argb) {#noticebuildertype_setcolor_argb}

- **argb** { ColorInt (dataTypes.html#datatypes_colorint) }
- **returns** { NoticeBuilder (noticeBuilderType.html) }
...
## [m] setContentTitle {#noticebuildertype_m_setcontenttitle}

### setContentTitle(title) {#noticebuildertype_setcontenttitle_title}

- **title** { string (dataTypes.html#datatypes_string) }
- **returns** { NoticeBuilder (noticeBuilderType.html) }

设定通知的文本标题.

## [m] setContentText {#noticebuildertype_m_setcontenttext}

### setContentText(text) {#noticebuildertype_setcontenttext_text}
...
## [m] setOnGoing {#noticebuildertype_m_setongoing}

### setOnGoing(ongoing) {#noticebuildertype_setongoing_ongoing}

- **ongoing** { boolean (dataTypes.html#datatypes_boolean) }
- **returns** { NoticeBuilder (noticeBuilderType.html) }

设定通知为 "正在进行中" 状态.

正在进行中, 意味着通知关联着一个用户正在参与的后台任务, 如 [ 播放音乐 / 下载任务 / 文件同步操作 / 网络连接激活 ] 等.
这样的通知不能被用户消除 (如左右滑动), 只能通过 notice.cancel (notice.html#notice_m_cancel) 或 NoticeBuilder#setAutoCancel (noticeBuilderType.html#noticebuildertype_m_setautocancel) 等方式消除.

## [m] setProgress {#noticebuildertype_m_setprogress}

### setProgress(max, progress, indeterminate) {#noticebuildertype_setprogress_max_progress_indeterminate}

- **max** { number (dataTypes.html#datatypes_number) } - 进度最大值
- **progress** { number (dataTypes.html#datatypes_number) } - 当前进度值
- **indeterminate** { boolean (dataTypes.html#datatypes_boolean) } - 是否为不确定进度条
- **returns** { NoticeBuilder (noticeBuilderType.html) }
...
## [m] setSmallIcon {#noticebuildertype_m_setsmallicon}

### setSmallIcon(icon) {#noticebuildertype_setsmallicon_icon}

**`Overload 1/2`**

- **icon** { number (dataTypes.html#datatypes_number) } - Drawable (可绘制) 资源 ID (glossaries.html#glossaries_资源_ID)
- **returns** { NoticeBuilder (noticeBuilderType.html) }

设定通知的小图标.

`icon` 参数需对应内置的 Drawable 资源, 使用 R.drawable (global.html#global_p_drawable) 可获取 AutoJs6 的 Drawable (可绘制) 资源 ID:
...
### setSmallIcon(iconCompat) {#noticebuildertype_setsmallicon_iconcompat}
...
## [m] setStyle {#noticebuildertype_m_setstyle}

### setStyle(style) {#noticebuildertype_setstyle_style}
...
#### BigTextStyle {#noticebuildertype_bigtextstyle}
...
NoticeOptions#bigContent (noticeOptionsType.html#noticeoptionstype_p_bigcontent) 接口属性, 其内部实现就使用了 BigTextStyle 样式模板类.

参阅: Android Docs (https://developer.android.com/reference/androidx/core/app/NotificationCompat.BigTextStyle)

#### MessagingStyle {#noticebuildertype_messagingstyle}
...
#### BigPictureStyle {#noticebuildertype_bigpicturestyle}
...
#### InboxStyle {#noticebuildertype_inboxstyle}
...
## [m] setTimeoutAfter {#noticebuildertype_m_settimeoutafter}

### setTimeoutAfter(duration) {#noticebuildertype_settimeoutafter_duration}

- **duration** { number (dataTypes.html#datatypes_number) } - 超时时间 (毫秒)
- **returns** { NoticeBuilder (noticeBuilderType.html) }
...
## [m] setUsesChronometer {#noticebuildertype_m_setuseschronometer}

### setUsesChronometer(b) {#noticebuildertype_setuseschronometer_b}

- **b** { boolean (dataTypes.html#datatypes_boolean) } - 是否使用通知计时秒表
- **returns** { NoticeBuilder (noticeBuilderType.html) }
...
## [m] setChronometerCountDown {#noticebuildertype_m_setchronometercountdown}

### setChronometerCountDown(countDown) {#noticebuildertype_setchronometercountdown_countdown}

- **countDown** { boolean (dataTypes.html#datatypes_boolean) }
- **returns** { NoticeBuilder (noticeBuilderType.html) }
...
## [m] setWhen {#noticebuildertype_m_setwhen}

### setWhen(when) {#noticebuildertype_setwhen_when}

- **when** { number (dataTypes.html#datatypes_number) } - 时间戳 (毫秒)
- **returns** { NoticeBuilder (noticeBuilderType.html) }

添加一个通知时间戳 (毫秒), 用以表示通知发生 (或即将发生) 的具体时间.

使用 `setWhen` 时, 需要设置 setShowWhen 为 `true`, 否则将无法显示时间消息.
...
## [m] setShowWhen {#noticebuildertype_m_setshowwhen}

### setShowWhen(show) {#noticebuildertype_setshowwhen_show}

- **show** { boolean (dataTypes.html#datatypes_boolean) } - 是否显示时间戳信息
- **returns** { NoticeBuilder (noticeBuilderType.html) }

设置通知消息是否显示使用 setWhen 设置的时间戳信息.

当 `setShowWhen` 设置为 `false` 时, setWhen 设置的时间戳信息将不再显示在通知消息中.
