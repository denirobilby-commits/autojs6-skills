...
title: "存储 (Storages)"
version: "6.6.4"
source_html: "raw/storages.html"
---

# 存储 (Storages) {#storages_storages}
...
- AutoJs6 应用被卸载或清除数据
- 使用 storages.remove / Storage#remove (storageType.html#storagetype_m_remove) / Storage#clear (storageType.html#storagetype_m_clear) 等方法删除
...
具体的存入规则详见 Storage#put (storageType.html#storagetype_m_put) 小节.
...
## [m] create {#storages_m_create}

### create(name) {#storages_create_name}

- **name** { string (dataTypes.html#datatypes_string) } - 存储名称
- **returns** { Storage (storageType.html) }

以 `name` 参数为名称创建一个本地存储, 并返回 Storage (storageType.html) 实例:
...
## [m] remove {#storages_m_remove}

### remove(name) {#storages_remove_name}

- **name** { string (dataTypes.html#datatypes_string) } - 存储名称
- **returns** { boolean (dataTypes.html#datatypes_boolean) } - name 参数对应的本地存储是否存在
