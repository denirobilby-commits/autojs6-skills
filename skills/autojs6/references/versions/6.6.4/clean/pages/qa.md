...
- 颜色 (colors) (color.html)
- 选择器 (UiSelector) (uiSelectorType.html)
- 控件节点 (UiObject) (uiObjectType.html)
- ... ...

其中尤其具备 AutoJs6 特色的, 当属 pickup 选择器 (uiSelectorType.html#uiselectortype_m_pickup) 及 compass 控件罗盘 (uiObjectType.html#uiobjecttype_m_compass).
...
参阅: 光学字符识别 (OCR) (ocr.html) 模块

### 区域截图 {#qa_5}

AutoJs6 不支持区域截图.

可通过 images.captureScreen (image.html#image_m_capturescreen) 截取屏幕后使用 images.clip (image.html#image_m_clip) 等方法做进一步处理.
...
- notice (notice.html) 模块的渠道相关功能只能在 `Android API 26 (8.0) [O]` 及以上起作用
- device.imei (device.html#device_p_imei) 只能在 `Android API 29 (10) [Q]` 及以下获取到设备 IMEI 值
- UiSelector#imeEnter (uiSelectorType.html#uiselectortype_m_imeenter) 只能在 `Android API 30 (11) [R]` 及以上才能起作用
- UiSelector#dragStart (uiSelectorType.html#uiselectortype_m_dragstart) 只能在 `Android API 32 (12.1) [S_V2]` 及以上才能起作用
- UiSelector#showTextSuggestions (uiSelectorType.html#uiselectortype_m_showtextsuggestions) 只能在 `Android API 33 (13) [TIRAMISU]` 及以上才能起作用
- ... ...
...
厂商或品牌 操作系统 魅族 (MEIZU) Flyme OS 欧珀 (OPPO / Realme) ColorOS 小米 (Xiaomi / Redmi / BlackShark) MIUI / HyperOS 一加 (OnePlus) 氢OS / Oxygen OS 维沃 (VIVO / IQOO) Funtouch OS / OriginOS 华为 (Huawei / Honor) EMUI / HarmonyOS 联想 (Lenovo) ZUI 酷派 (Coolpad) CoolOS 卓易 (Droi) Freeme OS 锤子科技 (Smartisan) Smartisan OS 中兴 (ZTE / 天机 / 远航 / Axon) MyOS 努比亚 (Nubia / 红魔) REDMAGIC OS Google Pixel 原生 AVD (安卓虚拟机) 原生 索尼 (Sony / XPERIA) 类原生 三星 (Samsung) 类原生 黑莓 (BlackBerry) 类原生 LG 类原生 摩托罗拉 (Motorola) 类原生 诺基亚 (Nokia) 类原生 (仅限部分机型) 华硕 (ASUS / ZenFone / ROG Phone) 类原生 宏达电 (HTC) 类原生
...
关于脚本化 Java 的更多信息, 参阅 Scripting Java - 脚本化 Java (scriptingJava.html) 章节.
...
关于 **应用文档** 的反馈:
http://docs-issues.autojs6.com (http://docs-issues.autojs6.com)

关于 **AutoJs6** 的反馈:
http://issues.autojs6.com (http://issues.autojs6.com)
